...
public class ClassesVariaveisServiceTest {
    ...
    private ClassesVariaveisDTO montarDtoOK() {
        ...
        dtoOK.setId(id);
        dtoOK.setId(123L);
        dtoOK.setVariavel("aaaaaaaaaa");
        dtoOK.setTipo("aaaaaaaaaa");
        dtoOK.setStatus(Status.ATIVO);
        dtoOK.setHeader(dtoOK.getVariavel());
        return dtoOK;
    }
    ...
    private ClassesVariaveisDTO montarDtoErroMin() {
        ...
        dtoErroMin.setVariavel("aaaaaaaaa");
        dtoErroMin.setTipo("aaaaaaaaa");
    ...
    public void tentarSalvarErroRepetido() {
        ...
        classesVariaveis.setId(idErro);

        List<ClassesVariaveis> classesVariaveiss = new ArrayList<>();
        classesVariaveiss.add(classesVariaveis);
        Page<ClassesVariaveis> msPage = new PageImpl<>(classesVariaveiss);

        Mockito.when(classesVariaveisRepository.findAll(Mockito.any(Specification.class), Mockito.any(Pageable.class))).thenReturn(msPage);
        try {
            service.save(dtoOk);
        } catch (BusinessRuleException e) {
            Assertions.assertEquals("Já existe este classesVariaveis cadastrado", e.getMessage());
        }
    }
    ...
    public void tentarSalvarOK() {
        ...
        Collection<Invocation> invocations = Mockito.mockingDetails(classesVariaveisRepository).getInvocations();
    ...
    public void updateErro() {
        ...
        Mockito.when(classesVariaveisRepository.findAll(Mockito.any(Specification.class), Mockito.any(Pageable.class))).thenReturn(msPage);
        Mockito.when(classesVariaveisRepository.findById(id)).thenReturn(Optional.of(classesVariaveis));
        Mockito.when(classesVariaveisRepository.save(Mockito.any())).thenThrow(new RuntimeException("erro entity"));
    ...
    public void montarDTOOk() {
        ...
        ClassesVariaveisDTO n = new ClassesVariaveisDTO();

        ClassesVariaveisDTO novo = service.montarDTO(classesVariaveis);
